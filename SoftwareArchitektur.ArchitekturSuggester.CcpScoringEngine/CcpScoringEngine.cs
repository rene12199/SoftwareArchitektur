...
public class CcpScoringEngine : ICcpScoringEngine
{
    ...
    private int RegisterServiceInBestPackage(List<Tuple<ServiceModel, List<CcpScoringCommonChangeClass>>> remainingServicesTuple, CcpScoringCommonChangeClass? bestPackage,
        int iterator)
    {
    ...
    private bool CheckIfServiceHasChanges(List<Tuple<ServiceModel, List<CcpScoringCommonChangeClass>>> remainingServicesTuple, int iterator)
    {
    ...
    private async Task CalculateChangesByPackageAsync(IEnumerable<Tuple<ServiceModel, List<CcpScoringCommonChangeClass>>> remainingServicesTuple)
    {
        await Parallel.ForEachAsync(remainingServicesTuple, (tuple, token) =>
        {
            ...
            UpdateServiceTuple(tuple);
            return ValueTask.CompletedTask;
        });
    }

    private void UpdateServiceTuple(Tuple<ServiceModel, List<CcpScoringCommonChangeClass>> tuple)
    {
        tuple?.Item2?.Clear();
    ...
    private static byte[] CreateShaFromRemainingServices(IList<ServiceModel> remainingServices)
    {
        ...
        using (SHA256 sha256Hash = SHA256.Create())
        {
            ...
            foreach (var remainingService in remainingServices) sb.Append(remainingService.Name);
