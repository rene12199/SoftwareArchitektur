...
public class LayeringEngineTests
{
    ...
    [Test]
    public void LayeringEngineTests_EvenLayering_ShallowGraph_ReferenceInSameLevel_Returns3LayersWith3Packages()
    {
        // Arrange
        var groupPackageModels = new List<GroupingPackageModel>();
        var packageModels = new List<PackageModel>();
        ...
        for (int i = 9; i > 0; i--)
        {
            ...
            var dependsOn = packageModels.FirstOrDefault(x => x.PackageName == $"P{i + 3}");
            if (packageModel.PackageName == "P8")
            {
                groupingDependencies.Add(new GroupingDependendencyModel(packageModel, packageModels.FirstOrDefault(x => x.PackageName == $"P9"), 1));
            }
            if (dependsOn != null)
            {
                var dependency = new GroupingDependendencyModel(packageModel, dependsOn, 1);
        ...
        Assert.That(result.Count(m => m.Layer == 3), Is.EqualTo(1));
    }
